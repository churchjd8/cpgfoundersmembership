      ...
      <div className="mt-8 rounded-xl bg-accent-light/60 p-5">
        ...
        <div className="mt-3 flex flex-wrap items-baseline gap-x-3 gap-y-1">
          ...
          <span className="text-lg text-muted">
            &times; {ENGAGEMENT.initialMonths} months — $
            {ENGAGEMENT.totalInitialUsd.toLocaleString()} total
          </span>
        ...
        <p className="mt-3 text-foreground leading-relaxed">
          ${ENGAGEMENT.initialFeeUsd.toLocaleString()}/month across the three
          months, or paid in full up front if that&apos;s easier on the books.
          Either way it&apos;s the same $
          {ENGAGEMENT.totalInitialUsd.toLocaleString()} total. After the three
          months, you can continue month-to-month at $
          {ENGAGEMENT.ongoingLowUsd.toLocaleString()}–$
          {ENGAGEMENT.ongoingHighUsd.toLocaleString()}/month depending on how much
          of Jeff&apos;s time makes sense for where {ENGAGEMENT.business} is
          headed. We&apos;ll set that together before the term ends.
        </p>
      </div>

      <div className="mt-6 rounded-xl border-2 border-accent bg-background p-5">
        <h3 className="text-sm font-bold uppercase tracking-wider text-accent">
          The ROI guarantee
        </h3>
        <p className="mt-3 text-foreground leading-relaxed">
          If you don&apos;t see a tangible return in three months, Jeff keeps
          working with you for free until you do, or refunds you 100%. What we ask
          in return is that you show up to every call with the pre-work done, and
          that you actually run the strategy rather than letting it sit in a doc.
          It&apos;s written into the agreement on the next step.
        </p>
      </div>
...
function AgreementStep({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch("/api/abv-onboarding/sign", {
        method: "POST",
  ...
  return (
    <Card>
      ...
      <p className="mt-2 text-sm text-muted">
        ...
        <strong className="text-foreground">{ENGAGEMENT.business}</strong>.
        Effective {ENGAGEMENT.effectiveDateLabel}. Whichever of you signs is
      ...
      <label className="mt-5 flex items-start gap-3 text-sm text-muted">
        ...
        />
        <span>
          I, {fullName || "the undersigned"}, am authorized to sign on behalf of{" "}
          {ENGAGEMENT.business}, and have read and agree to this Advisory Services
          Agreement. I understand my signature here is the legal equivalent of a
          handwritten signature and authorizes the payment described in the next
          step.
        </span>
      </label>
...
function PaymentStep({ onBack }: { onBack: () => void }) {
  ...
  async function handlePay() {
    ...
    try {
      ...
      try {
        stash = JSON.parse(sessionStorage.getItem("abvOnboarding") || "{}");
      } catch {
        stash = {};
      }
      const res = await fetch("/api/abv-onboarding/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <Card>
      ...
      <p className="mt-2 text-muted leading-relaxed">
        Last step. Both options come to the same $
        {ENGAGEMENT.totalInitialUsd.toLocaleString()} total. Pick whichever works
        better for your cash flow, then you&apos;ll be taken to our secure Stripe
        checkout to enter your card or bank details. Billing starts today.
      </p>
      ...
      <p className="mt-4 text-sm text-muted">
        ...
        month-to-month at $
        {ENGAGEMENT.ongoingLowUsd.toLocaleString()}–$
        {ENGAGEMENT.ongoingHighUsd.toLocaleString()}/month at a rate we&apos;ll set
        together before the term ends, and cancel anytime with{" "}
        {ENGAGEMENT.cancelNoticeDays} days&apos; notice.
      </p>
