    ...
    <Card>
      ...
      <div className="mt-8 rounded-xl bg-accent-light/60 p-5">
        ...
        <p className="mt-2 text-foreground leading-relaxed">
          ${ENGAGEMENT.initialFeeUsd.toLocaleString()}/month for the{" "}
          {ENGAGEMENT.initialMonths}-month intensive ($
          {ENGAGEMENT.totalInitialUsd.toLocaleString()} total). After that, you can
          continue month-to-month at a rate we&apos;ll set together before the term
          ends, based on how much of Jeff&apos;s time makes sense for where Tulua is
          headed.
        </p>
      </div>
...
function AgreementStep({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch("/api/zeyad-onboarding/sign", {
        method: "POST",
...
function PaymentStep({ onBack }: { onBack: () => void }) {
  ...
  async function handlePay() {
    ...
    try {
      ...
      try {
        stash = JSON.parse(sessionStorage.getItem("zeyadOnboarding") || "{}");
      } catch {
        stash = {};
      }
      const res = await fetch("/api/zeyad-onboarding/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <div className="mt-6 rounded-xl border border-border bg-background p-5">
        <div className="flex items-baseline justify-between">
          <span className="font-medium">
            Intensive, months 1&ndash;{ENGAGEMENT.initialMonths}
          </span>
        ...
        <p className="mt-2 text-sm text-muted">
          ...
          commitment (${ENGAGEMENT.totalInitialUsd.toLocaleString()} total), then
          month-to-month at a rate we set together before the term ends. Cancel
          anytime after the first {ENGAGEMENT.initialMonths} months with{" "}
