  ...
  return (
    <Card>
      ...
      <p className="mt-4 text-muted leading-relaxed">
        We&apos;ve tightened up how Jeff works with his founders, so here&apos;s
        the current overview before we make it official. Three quick steps: review
        what&apos;s included, sign the agreement, and set up payment. Should take
        about five minutes.
      </p>
      ...
      <div className="mt-8 rounded-xl bg-accent-light/60 p-5">
        <h3 className="font-semibold text-accent-dark">Your founding rate</h3>
        <p className="mt-2 text-foreground leading-relaxed">
          ${ENGAGEMENT.initialFeeUsd.toLocaleString()}/month for your first{" "}
          {ENGAGEMENT.initialMonths} months, then $
          {ENGAGEMENT.ongoingFeeUsd.toLocaleString()}/month, month-to-month after
          that. New founders are coming in at higher rates this summer. Because we
          connected before that, this rate is yours.
        </p>
      </div>
...
function AgreementStep({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch("/api/jade-onboarding/sign", {
        method: "POST",
    ...
    <Card>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <label className="block">
          ...
          <input
            ...
            placeholder="you@fermenteria.com"
            className="mt-1 w-full rounded-lg border border-border bg-white px-3 py-2 text-foreground outline-none focus:border-accent focus:ring-1 focus:ring-accent"
          />
        </label>
      </div>
      ...
      <label className="mt-5 flex items-start gap-3 text-sm text-muted">
        ...
        <span>
          ...
          of a handwritten signature and authorizes the recurring Stripe subscription
...
function PaymentStep({ onBack }: { onBack: () => void }) {
  ...
  async function handlePay() {
    ...
    try {
      ...
      try {
        stash = JSON.parse(sessionStorage.getItem("jadeOnboarding") || "{}");
      } catch {
        stash = {};
      }
      const res = await fetch("/api/jade-onboarding/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <Card>
      ...
      <p className="mt-2 text-muted leading-relaxed">
        Last step. You&apos;ll be taken to our secure Stripe checkout to enter your
        card or bank details. Billing starts today.
      </p>

      <div className="mt-6 rounded-xl border border-border bg-background p-5">
        <div className="flex items-baseline justify-between">
          <span className="font-medium">
            Founding rate, months 1&ndash;{ENGAGEMENT.initialMonths}
          </span>
        ...
        <p className="mt-2 text-sm text-muted">
          Billed monthly via Stripe for your {ENGAGEMENT.initialMonths}-month
          commitment, then ${ENGAGEMENT.ongoingFeeUsd.toLocaleString()}/month
          month-to-month. Cancel anytime after the first {ENGAGEMENT.initialMonths}{" "}
          months with {ENGAGEMENT.cancelNoticeDays} days&apos; notice.
        </p>
      </div>
