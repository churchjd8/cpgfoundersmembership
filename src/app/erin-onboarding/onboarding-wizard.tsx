    ...
    <Card>
      ...
      <div className="mt-8 rounded-xl bg-accent-light/60 p-5">
        ...
        <p className="mt-2 text-foreground leading-relaxed">
          ${ENGAGEMENT.totalInitialUsd.toLocaleString()} for the{" "}
          {ENGAGEMENT.initialMonths}-month intensive, billed at $
          {ENGAGEMENT.initialFeeUsd.toLocaleString()}/month. Prefer to pay in full?
          It&apos;s ${ENGAGEMENT.payInFullUsd.toLocaleString()} up front and you save
          ${ENGAGEMENT.payInFullSavingsUsd.toLocaleString()}. After the three months,
          most founders continue month-to-month at $
          {ENGAGEMENT.ongoingFeeUsd.toLocaleString()}/month, cancel anytime.
        </p>
      </div>
...
function AgreementStep({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch("/api/erin-onboarding/sign", {
        method: "POST",
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <label className="block">
          ...
          <input
            ...
            placeholder="erin@dropsofdough.com"
            className="mt-1 w-full rounded-lg border border-border bg-white px-3 py-2 text-foreground outline-none focus:border-accent focus:ring-1 focus:ring-accent"
          />
        </label>
      </div>
...
function PaymentStep({ onBack }: { onBack: () => void }) {
  ...
  async function handlePay() {
    ...
    try {
      ...
      try {
        stash = JSON.parse(sessionStorage.getItem("erinOnboarding") || "{}");
      } catch {
        stash = {};
      }
      const res = await fetch("/api/erin-onboarding/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <Card>
      ...
      <p className="mt-2 text-muted leading-relaxed">
        Last step. Pick the option that works best for you, then you&apos;ll be taken
        to our secure Stripe checkout to enter your card or bank details. Billing
        starts today.
      </p>
          ...
          <div className="flex items-baseline justify-between">
            <span className="font-medium">
              ...
              <span className="text-xs font-semibold text-accent-dark">
                save ${ENGAGEMENT.payInFullSavingsUsd.toLocaleString()}
              </span>
            </span>
            <span className="font-serif text-2xl font-bold">
              ${ENGAGEMENT.payInFullUsd.toLocaleString()}
            </span>
          </div>
          <p className="mt-2 text-sm text-muted">
            One payment up front for the full {ENGAGEMENT.initialMonths}-month
            intensive. Save ${ENGAGEMENT.payInFullSavingsUsd.toLocaleString()}.
          </p>
        </button>
      </div>

      <p className="mt-4 text-sm text-muted">
        After the {ENGAGEMENT.initialMonths}-month intensive, most founders continue
        month-to-month at ${ENGAGEMENT.ongoingFeeUsd.toLocaleString()}/mo. Cancel
        anytime with {ENGAGEMENT.cancelNoticeDays} days&apos; notice. We&apos;ll set
        that up together before the term ends.
      </p>
