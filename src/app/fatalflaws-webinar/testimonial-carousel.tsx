          ...
          <figure key={testimonial.name} className="flex min-h-[310px] w-[88%] shrink-0 snap-start flex-col rounded-2xl border border-border bg-white p-7 shadow-sm sm:w-[64%] lg:w-[42%]">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-full border-2 border-accent/20 bg-accent-light">
                  <Image src={testimonial.image} alt={testimonial.name} fill className="object-cover" sizes="64px" />
                </div>
                <div>
                  <p className="font-bold text-foreground">{testimonial.name}</p>
                  <p className="mt-1 text-xs font-bold uppercase tracking-[0.14em] text-accent">{testimonial.tag}</p>
                </div>
              </div>
            ...
            <figcaption className="mt-8 border-t border-border pt-5">
              <p className="text-sm leading-relaxed text-muted">{testimonial.title}</p>
            </figcaption>
          </figure>
        ))}
      </div>
