...
// Records ABV's signed agreement by emailing a durable record (with the
// signature image attached) to the team. Mirrors the Resend-via-fetch pattern
// used elsewhere in this app.
export async function POST(request: Request) {
  try {
    const { fullName, title, email, signatureImage, agreementVersion, signedAt } =
      await request.json();
        ...
        body: JSON.stringify({
          ...
          html: `
            ...
            <table style="border-collapse:collapse;font-family:sans-serif;">
              <tr><td style="padding:8px;font-weight:bold;">Signer</td><td style="padding:8px;">${fullName}${title ? ` — ${title}` : ""}</td></tr>
              ...
              <tr><td style="padding:8px;font-weight:bold;">Terms</td><td style="padding:8px;">$${ENGAGEMENT.totalInitialUsd} for ${ENGAGEMENT.initialMonths} months — 3-Month VIP Intensive at standard rate ($${ENGAGEMENT.initialFeeUsd}/mo or $${ENGAGEMENT.totalInitialUsd} paid in full), then month-to-month at $${ENGAGEMENT.ongoingLowUsd}–$${ENGAGEMENT.ongoingHighUsd}/mo agreed before the term ends. ROI guarantee applies.</td></tr>
            </table>
    ...
    console.error("ABV onboarding sign error:", err);
    return NextResponse.json(
