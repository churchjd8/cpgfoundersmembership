...
async function sendNotificationEmail(fields: { firstName: string; email: string }) {
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      subject: `Vetted Services provider application: ${fields.firstName}`,
      html: `
        <h2>New Vetted Services Provider Application</h2>
...
export async function POST(request: Request) {
  try {
        ...
        body: JSON.stringify({
          data: {
            ...
            attributes: {
              name: firstName,
              email,
              custom_3: "Type: Provider application",
            },
          },
        }),
      },
    ...
    if (!formRes.ok) {
      ...
      console.error(`Vetted services apply Kajabi submit failed (${formRes.status}): ${errText.slice(0, 300)}`);
    }
    ...
    console.error("Vetted services apply error:", err);
